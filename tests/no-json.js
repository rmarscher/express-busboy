 ...
 *
 * This test suite tests the option to turn off json body parsing
 */
...
var tests = {
    'setup hack': {
        ...
        'and should not handle bad json': {
            topic: function() {
                var done = this.callback;
                var req = request({
                    method: 'POST',
        ...
        'and should not parse post body with JSON': {
            topic: function() {
                var done = this.callback;
                request({
                    method: 'POST',
...
vows.describe('express-busboy: no-json').addBatch(tests).export(module);
